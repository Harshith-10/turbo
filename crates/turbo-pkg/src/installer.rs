...
impl Installer {
    ...
    pub async fn install(&self, def: &PackageDefinition) -> anyhow::Result<()> {
        ...
        if install_dir.exists() {
            tracing::info!("Package {}@{} is already installed at {:?}", pkg_name, pkg_version, install_dir);
            return Ok(());
        }

        tracing::info!("Installing {}@{} from {:?}", pkg_name, pkg_version, def.path);

        // 1. Run build.sh
        // Canonicalize path to ensure reliable execution independent of CWD
        let abs_pkg_path = def.path.canonicalize().map_err(|e| anyhow::anyhow!("Failed to canonicalize package path {:?}: {}", def.path, e))?;
        ...
        fs::copy(abs_pkg_path.join("package.yaml"), install_dir.join("package.yaml")).await?;
